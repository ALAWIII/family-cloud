# Refresh Token Integration Tests (Refactored)
...
async fn setup_with_authenticated_user() -> anyhow::Result<(AppTest, family_cloud::AppState, TestAccount, LoginResponse)> {
...
async fn test_refresh_can_be_used_for_authenticated_requests() -> anyhow::Result<()> {
    ...
    assert!(
        !auth_response.status().is_unauthorized(),
...
async fn test_concurrent_refreshes_succeed() -> anyhow::Result<()> {
    ...
    let handle1 = tokio::spawn({
        let app = app.clone();
        async move {
            app.refresh_with_body(&body1).await.status().is_success()
        }
    });
    ...
    let handle2 = tokio::spawn({
        let app = app.clone();
        async move {
            app.refresh_with_body(&body2).await.status().is_success()
        }
    });
...
|----------|----------|---------|
| `login(None, None).await?` | `setup_with_authenticated_user().await?` | Clear, returns all needed data |
| `loginres.json()` → manual LoginResponse | Direct `LoginResponse` from helper | Type-safe, cleaner |
| `refresh_token_body_cookie()` | `create_token_pair()` | Clearer naming |
| `app.refresh_body_request()` | `app.refresh_with_body()` | Consistent naming |
| `app.refresh_cookie_request()` | `app.refresh_with_cookie()` | Consistent naming |
| `app.refresh_non_request()` | `app.refresh()` | Shorter, clearer |

---

## Tests Included

✅ **test_refresh_with_token_in_body** - Desktop/mobile pattern  
✅ **test_refresh_with_token_in_cookie** - Web browser pattern  
✅ **test_refresh_without_token** - Validation (should fail)  
✅ **test_refresh_generates_new_access_token** - Token generation  
✅ **test_refresh_with_body_and_cookie_different_tokens** - Multiple refresh methods  
✅ **test_refresh_with_invalid_token** - Invalid token handling  
✅ **test_refresh_multiple_times_in_sequence** - Repeated refreshes  
✅ **test_refresh_returns_valid_jwt** - JWT format validation  
✅ **test_refresh_can_be_used_for_authenticated_requests** - Integration test  
✅ **test_refresh_after_logout_fails** - Token invalidation  
✅ **test_concurrent_refreshes_succeed** - Concurrency handling  
...
### Better Structure
- Shared `setup_test_env()` used across tests
- `setup_with_authenticated_user()` helper for login flow
- Clear separation of concerns

### Enhanced Test Coverage
- **7 new tests** added (beyond original)
- Edge cases: invalid tokens, concurrent requests, post-logout
- Integration: using new token for authenticated endpoints
- Format validation: JWT structure checking
