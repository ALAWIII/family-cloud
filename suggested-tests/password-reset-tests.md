# Password Reset Integration Tests (Refactored)
...
use family_cloud::{
    TokenType, create_verification_key, get_db, get_redis_con, is_token_exist,
};
...
async fn setup_with_verified_account() -> anyhow::Result<(AppTest, family_cloud::AppState, TestAccount)> {
...
async fn request_reset_and_get_tokens(
    ...
    .iter()
    .map(|v| create_verification_key(TokenType::PasswordReset, v))
    .collect::<Vec<_>>();
...
async fn test_password_reset_retrieves_form() -> anyhow::Result<()> {
    ...
    for raw_token in &raw_tokens {
        let response = app.verify_email("/api/auth/password-reset", raw_token).await;
        assert!(
            response.status().is_success(),
...
async fn test_password_reset_rejects_mismatched_passwords() -> anyhow::Result<()> {
    ...
    for raw_token in &raw_tokens {
        let response = app.confirm_password_reset(
            raw_token,
            "NewPassword123!",
...
async fn test_password_reset_token_single_use_only() -> anyhow::Result<()> {
    ...
    for raw_token in &raw_tokens {
        let first_use = app.confirm_password_reset(
            raw_token,
            "FirstNewPassword123!",
            "FirstNewPassword123!",
        ).await;
        ...
        assert!(
            first_use.status().is_success(),
            "First password reset should succeed"
        );

        // Try reusing the SAME token
        let reuse_attempt = app.confirm_password_reset(
            raw_token,
...
async fn test_password_reset_allows_login_with_new_password() -> anyhow::Result<()> {
    ...
    // Reset password
    for raw_token in &raw_tokens {
        app.confirm_password_reset(raw_token, new_password, new_password).await;
    }
...
|----------|----------|---------|
| `setup_app()` | `setup_test_env()` | Returns AppState explicitly |
| `create_verified_account()` | `TestDatabase::create_verified_account()` | Better naming, encapsulated |
| `app.get_all_messages_mailhog()` | `app.mailhog.get_all_messages().await?` | Cleaner API, error handling |
| `app.click_verify_url_in_email_message()` | `app.verify_email()` | Shorter, more readable |
| `extract_raw_token_list()` | `EmailTokenExtractor::extract_raw_tokens()` | Encapsulated, testable |
| `convert_raw_tokens_to_hashed()` | `EmailTokenExtractor::hash_tokens()` | Clear intent, error handling |
| Manual redis setup | Passed via `setup_with_verified_account()` | Centralized, reusable |

---

## New Tests Added

✅ **test_password_reset_token_single_use_only** - Ensures tokens can't be reused  
✅ **test_password_reset_allows_login_with_new_password** - Integration with login  
✅ **test_password_reset_prevents_old_password_login** - Old password becomes invalid  
✅ **test_password_reset_for_nonexistent_email** - Security: no email enumeration
